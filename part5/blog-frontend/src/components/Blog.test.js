...
describe('<Blog />', () => {
  ...
  test('renders title and author by default but not url or number of likes', () => {
    ...
    container = render(<Blog blog={blog} blogs={[]} setBlogs={ () => null } isUserBlog={true} />).container
  ...
  test('clicking the button "like" of a blog twice calls the event handler twice', async () => {
    ...
    container = render(<Blog blog={blog} likeHandle={mockLikeHandler} blogs={[]} setBlogs={ () => null } isUserBlog={true} />).container
    const user = userEvent.setup()
